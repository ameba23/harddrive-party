...
pub struct MqttClient {
    topics: Arc<Mutex<HashSet<MqttTopic>>>,
}
...
impl MqttClient {
    pub async fn new(
        ...
        let topics_set: HashSet<MqttTopic> = topics
            .into_iter()
            .filter_map(move |topic| {
                MqttTopic::new(topic, true, true, &announce_address, &client_id).ok()
            })
            .collect();
        let mqtt_client = Self {
            topics: Arc::new(Mutex::new(topics_set)),
        };

        mqtt_client
            .run(
                client_id_clone,
                announce_address_clone,
                peers_tx,
                hole_puncher,
            )
            .await?;
    ...
    pub async fn run(
        ...
        let mut connect_packet = ConnectPacket::new(client_id.clone());
        ...
        // Subscribe to given topics
        {
            ...
            let channel_filters: Vec<(TopicFilter, QualityOfService)> = topics
                ...
                .map(|t| {
                    (
                        TopicFilter::new(format!("hdp/{}/#", t.topic.public_id)).unwrap(),
            ...
            loop {
                tokio::select! {
                                    ...
                                    match packet {
                                        ...
                                        VariablePacket::PublishPacket(ref publ) => {
                                            ...
                                            if let Some(associated_topic) = topics.iter().find(|&topic| {
                                                ...
                                                tn.contains(&topic.topic.public_id)
                                            }) {
                                                if let Some(remote_peer_announce) = decrypt_using_topic(&publ.payload().to_vec(), &associated_topic.topic) {
                                                    ...
                                                    if should_initiate_connection {
                                                        ...
                                                        if peers_tx
                                                            .send(DiscoveredPeer {
                                                                ...
                                                                topic: Some(associated_topic.topic.clone()),
                                                            })
                                                            .is_err()
                                                        {
                                ...
                                () = tokio::time::sleep(Duration::from_secs(KEEP_ALIVE as u64 /2)) => {
                                    ...
                                    info!("Sending publish packet");
                                    let topics = topics_mutex.lock().await;
                                    // For each topic in the set, send publish message
                                    for topic in topics.iter() {
                                        if let Some(publish_packet) = &topic.publish_packet {
    ...
    pub fn add_topic(&self, _topic: Topic) {}

    pub fn remove_topic(&self, _topic: Topic) {}
}
